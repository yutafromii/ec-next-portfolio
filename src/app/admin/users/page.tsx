...
export default function AdminUserList() {
  ...
  const missingAddress = useMemo(
    () => users.filter((u) => !u.address)?.length ?? 0,
    [users]
  );
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      console.error(err);
      alert(
        "削除に失敗しました。注文履歴やカート等で参照されている可能性があります。"
      );
    }
  };
  ...
  if (error)
    return (
      <section className="max-w-7xl mx-auto py-10 mt-10 text-red-500">
        {error}
      </section>
    );
  ...
  return (
    <section className="py-8 mt-12">
      ...
      <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>住所未登録</CardTitle>
            <CardDescription>address が空</CardDescription>
          </CardHeader>
          <CardContent className="text-3xl font-semibold">
            {missingAddress}
          </CardContent>
      ...
      <div className="max-w-6xl mx-auto px-6 mb-4">
        ...
        <div className="text-sm text-gray-600 mt-2">
          該当 {filtered.length} 件（ページ {page + 1}/{Math.max(totalPages, 1)}
          ）
        </div>
      </div>
            ...
            <tbody>
              {filtered.map((u) => (
                <tr key={u.id} className="border-t hover:bg-gray-50">
                  ...
                  <td className="px-3 py-2">
                    {[u.prefecture, u.city, u.addressLine1, u.addressLine2]
                      .filter(Boolean)
                      .join(" ") ||
                      u.address ||
                      "未登録"}
